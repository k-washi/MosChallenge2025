...
log_params = {
    ...
    "metric": "rmse",
    "learning_rate": 0.03,
    "n_estimators": 10000,  # early_stopping ありきで多めに
    "num_leaves": 16,
    "max_depth": 5,
...
for i, csv_files in enumerate(csv_list):
    ...
    for j, csv_file in enumerate(csv_list):
        ...
        elif (i + 1) % (len(csv_list) - 1) == j:
            val_csv_list.append(csv_file)
        else:
    ...
    val_sample_id_list, val_true_mos_list, val_pred_mos_list = load_csv_list(val_csv_list)
    ...
    print(f"val_true_mos_list.shape: {val_true_mos_list.shape}")
    print(f"val_pred_mos_list.shape: {val_pred_mos_list.shape}")
    assert train_pred_mos_list.shape[0] > 0, f"train_pred_mos_list is empty for {csv_files}"
    assert val_pred_mos_list.shape[0] > 0, f"val_pred_mos_list is empty for {csv_files}"
    assert test_pred_mos_list.shape[0] > 0, f"test_pred_mos_list is empty for {csv_files}"
    # ----------------------------
    model = LGBMRegressor(**log_params)
    model.fit(
        train_pred_mos_list,
        train_true_mos_list,
        eval_set=[(val_pred_mos_list, val_true_mos_list)],
        eval_metric="rmse",
        callbacks=[
