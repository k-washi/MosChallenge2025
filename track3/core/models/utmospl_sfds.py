...
class MOSPredictorModule(LightningModule):
    ...
    def forward(self, x: torch.Tensor, ds_id: torch.Tensor, attention_mask: torch.Tensor | None = None) -> torch.Tensor:
        ...
        pred = self.model(x, ds_id=ds_id, attention_mask=attention_mask)
        if pred.dim() == OUTPUT_SQ_DIM:
            pred = pred.squeeze(1)
        return pred

    def _caculate_loss(
        self, pred1: torch.Tensor, mos_score1: torch.Tensor, ds_id: torch.Tensor | None = None, is_train: bool = False
    ) -> tuple:
        ...
        ----
            ...
            ds_id (torch.Tensor | None): Dataset IDs for the first set of predictions.
        ...
        if ds_id is None:
            ...
            rank_loss = torch.zeros(1, device=pred1.device)
            diff_loss = torch.zeros(1, device=pred1.device)
            ds_id_values = self.c.data.train_dataset_dict.values() if is_train else self.c.data.test_dataset_dict.values()
            ds_id_values = sorted(list(set(ds_id_values)))
            for ds_id_v in ds_id_values:
                ds_id_mask = ds_id == torch.tensor(ds_id_v, device=ds_id.device)
                if ds_id_mask.sum() > 1:
                    pred1_ds = pred1[ds_id_mask]
                    mos_score1_ds = mos_score1[ds_id_mask]
                    rank_loss += ranknet_loss(pred1_ds, mos_score1_ds, margin=self.c.loss.contrastive_loss_margin)
                    diff_loss += mosdiff_loss(pred1_ds, mos_score1_ds, margin=self.c.loss.contrastive_loss_margin)
    ...
    def training_step(self, batch: dict, batch_idx: int) -> torch.Tensor:
        ...
        pred = self(wavs1, ds_id=ds_id, attention_mask=attention_mask1)

        l1loss, rank_loss, diff_loss = self._caculate_loss(pred, mos_score1, ds_id, is_train=True)
    ...
    def validation_step(self, batch: dict, batch_idx: int) -> torch.Tensor:
        ...
        """
        _ = batch_idx
        wavs1, attention_mask1, mos_score1, wav_fp_list, ds_ids = batch
        pred = self(wavs1, ds_id=ds_ids, attention_mask=attention_mask1)

        l1loss, rank_loss, diff_loss = self._caculate_loss(pred, mos_score1, ds_id=ds_ids, is_train=False)
        l1_rate = self.c.loss.l1_rate_min + (self.c.loss.l1_rate_max - self.c.loss.l1_rate_min) * (
            self.global_step / self.total_train_steps
        )
        loss = l1_rate * l1loss + self.c.loss.cl_rate * rank_loss + self.c.loss.diff_rate * diff_loss
