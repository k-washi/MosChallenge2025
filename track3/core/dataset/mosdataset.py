...
class MOSDataset(torch.utils.data.Dataset):
    ...
    and processing MOS (Mean Opinion Score) dataset.
    """
    def __init__(
        self, 
        config:Config, 
        ...
        """
        Args:
            data_list (list): List of tuples containing (audio_file, mos_score, user_id).
        ...
        assert len(audio_list) == len(label_list) == len(user_id_list), \
    ...
    def __getitem__(self, idx: int) -> tuple:
        ...
        audio, sr = load_wave(
            str(audio_file), 
            sample_rate=self.c.data.sample_rate,
            mono=True,
            is_torch=True
        )
        assert isinstance(audio, torch.Tensor), \
            f"Audio file {audio_file} is not a torch tensor."
    ...
    def collate_fn(batch):
        ...
        """
        wavs, scores, user_ids, audio_files = zip(*batch)
        wavs = list(wavs)
        max_len = max([wav.shape[-1] for wav in wavs])
        
        wave_tensor = torch.zeros((len(wavs), 1, max_len))
        score_tensor = torch.tensor(scores, dtype=torch.long)
        user_tensor = torch.tensor(user_ids, dtype=torch.long)
        for i, (wav, score, user_id) in enumerate(zip(wavs, scores, user_ids)):
