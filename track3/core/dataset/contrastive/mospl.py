...
from track3.core.dataset.contrastive.mosdataset import MOSDataset
...
class MOSDataModule(LightningDataModule):
    ...
    def __init__(
        ...
        train_contrastive_dataset_list: list[tuple[tuple[str | Path, float], tuple[str | Path, float]]],
        val_dataset_list: list[tuple[str | Path, float, str]],
        val_contrastive_dataset_list: list[tuple[tuple[str | Path, float], tuple[str | Path, float]]],
        test_dataset_list: list[tuple[str | Path, float, str]],
        test_contrastive_dataset_list: list[tuple[tuple[str | Path, float], tuple[str | Path, float]]],
    ) -> None:
        """Initialize the MOSDataModule."""
        super().__init__()
        self.c = config
        self.train_dataset_list = train_dataset_list
        self.train_contrastive_dataset_list = train_contrastive_dataset_list
        self.val_dataset_list = val_dataset_list
        self.train_contrastive_dataset_list = train_contrastive_dataset_list
        self.val_contrastive_dataset_list = val_contrastive_dataset_list
        self.test_dataset_list = test_dataset_list
        self.test_contrastive_dataset_list = test_contrastive_dataset_list
    ...
    def setup(self, stage: str | None = None) -> None:
        ...
        if stage == "fit" or stage is None:
            self.train_dataset = MOSDataset(
                ...
                contrastive_dataset_list=self.train_contrastive_dataset_list,
                is_transform=True,
                is_dataextend=self.c.data.is_extend,
            )
            self.val_dataset = MOSDataset(
                config=self.c,
                dataset_list=self.val_dataset_list,
                contrastive_dataset_list=self.val_contrastive_dataset_list,
                is_transform=False,
            )
        if stage == "test" or stage is None:
            self.test_dataset = MOSDataset(
                ...
                contrastive_dataset_list=self.test_contrastive_dataset_list,
                is_transform=False,
            )
