...
def format_process(
    ...
    data_list = [(item[0], item[2], item[1]) for item in data_list]
...
def format_dataset(dataset_dir, output_dir):
    """
    ...
    user_map = format_process(
        info_csv=str(data_dir /  "utt_16k.csv"),
        wav_dir=str(data_dir / "wav"),
        output_dir=str(output_dir),
        output_csv_name="utt_16k.csv"
    )

    user_map = format_process(
        info_csv=str(data_dir / "utt_24k.csv"),
        wav_dir=str(data_dir / "wav"),
        output_dir=str(output_dir),
        output_csv_name="utt_24k.csv",
        user_dict=user_map
    )
    
    user_map = format_process(
        info_csv=str(data_dir / "utt_48k.csv"),
        wav_dir=str(data_dir / "wav"),
        output_dir=str(output_dir),
        output_csv_name="utt_48k.csv",
        user_dict=user_map
    )
...
if __name__ == "__main__":
    ...
    format_dataset(args.dataset_dir, args.output_dir)